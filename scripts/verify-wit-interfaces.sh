...
for artifact in "$DEMO_CORE" "$DEMO_COMPONENT" "$SMOKE_CORE" "$MODULE_CORE" "$PACKAGE_MODULE"; do
  [[ -f "$artifact" ]] || {
...
wasm-tools validate "$MODULE_CORE"
wasm-tools validate "$PACKAGE_MODULE"
...
wasm-tools print "$MODULE_CORE" >"$TEMP_DIR/module-parent.wat"
wasm-tools print "$PACKAGE_MODULE" >"$TEMP_DIR/module.wat"
wasm-tools component wit "$DEMO_COMPONENT" >"$TEMP_DIR/component.wit"

for import in runtime graphics font-assets; do
  rg -Fq "(import \"oos:platform/$import@0.3.0\"" \
    "$TEMP_DIR/demo.wat"
done
for export in init event frame shutdown; do
  rg -Fq "(export \"oos:platform/lifecycle@0.3.0#$export\"" \
    "$TEMP_DIR/demo.wat"
done
...
for interface in \
  runtime graphics gles canvas device audio camera power vibrator wifi ip bluetooth modem codec \
  storage device-storage font-assets assets system-services; do
  rg -Fq "(import \"oos:platform/$interface@0.3.0\"" \
    "$TEMP_DIR/smoke.wat"
done
rg -Fq "(import \"oos:platform/modules@0.3.0\"" \
  "$TEMP_DIR/module-parent.wat"
rg -Fq '(export "oos:platform/module-api@0.3.0#invoke"' \
  "$TEMP_DIR/module.wat"
rg -Fq 'export oos:platform/lifecycle@0.3.0;' \
