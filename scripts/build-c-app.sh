...
MAIN_STACK_BYTES=${OOS_WASM_MAIN_STACK_BYTES:-524288}
WORKER_STACK_BYTES=${OOS_WASM_WORKER_STACK_BYTES:-2097152}
...
TOTAL_STACK_BYTES=$((2 * (MAIN_STACK_BYTES > WORKER_STACK_BYTES ? MAIN_STACK_BYTES : WORKER_STACK_BYTES)))
if (( MAIN_STACK_BYTES < 65536 || WORKER_STACK_BYTES < 65536 ||
      TOTAL_STACK_BYTES >= INITIAL_MEMORY_BYTES ||
...
if [[ -n "$BUILD_CONTAINER" && "${OOS_IN_DISTROBOX:-0}" != 1 ]]; then
  exec distrobox enter "$BUILD_CONTAINER" -- env OOS_IN_DISTROBOX=1 \
    OOS_WASM_MAIN_STACK_BYTES="$MAIN_STACK_BYTES" \
    OOS_WASM_WORKER_STACK_BYTES="$WORKER_STACK_BYTES" \
