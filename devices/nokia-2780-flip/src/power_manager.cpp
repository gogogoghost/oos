...
constexpr const char *kRtcWakePath = "/sys/class/rtc/rtc0/wakealarm";
...
bool PowerManager::initialize(const std::string &service_name) {
  shutdown();
  implementation_->service = power::IPower::getService(service_name);
  if (!implementation_->service) {
    implementation_->error = "Power HAL service not found: " + service_name;
    return false;
  }
...
bool PowerManager::initialized() const {
  return implementation_->service != nullptr && implementation_->uevent_fd >= 0;
}
...
int PowerManager::waitForBatteryEvent(int timeout_ms,
  ...
  for (size_t offset = 0; offset < static_cast<size_t>(size);) {
    ...
    const size_t length = strnlen(field, size - offset);
...
bool PowerManager::setInteractive(bool interactive) {
  if (!implementation_->service) {
    implementation_->error = "Power HAL is not initialized";
    return false;
  }
  const auto result = implementation_->service->setInteractive(interactive);
  if (!result.isOk()) {
    implementation_->error = "setInteractive transport failed: " +
                             std::string(result.description().c_str());
    return false;
  }
  implementation_->error.clear();
  return true;
...
bool PowerManager::scheduleRtcWake(int delay_seconds) {
  ...
  if (!writeText(kRtcWakePath, "0") ||
      !writeText(kRtcWakePath, "+" + std::to_string(delay_seconds))) {
    implementation_->error =
...
bool PowerManager::suspend(int graceful_timeout_ms) {
  ...
  if (autosuspend_force_suspend(graceful_timeout_ms) != 0) {
    implementation_->error =
...
FlipState PowerManager::queryFlipState(const char *input_directory) {
  ...
  while (dirent *entry = readdir(directory)) {
    ...
    if (ioctl(fd, EVIOCGNAME(name.size()), name.data()) >= 0 &&
        !std::strcmp(name.data(), "hall_sensor")) {
  ...
  if (state == FlipState::Unknown)
    implementation_->error = "hall_sensor EV_KEY device was not found";
  else
    implementation_->error.clear();
  return state;
}
